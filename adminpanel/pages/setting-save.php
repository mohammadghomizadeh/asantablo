...
<?php
    ...
    elseif(isset($_POST['submitsocial']))
    {
        ...
        if(!empty($twitter)){
            $db->run("UPDATE `content` SET `link` = '$twitter' WHERE `title` = 'twitter'");
        }
        else
        {
            $db->run("UPDATE `content` SET `link` = 'لینک مورد نظر را وارد نمایید' WHERE `title` = 'twitter'");
        }
        $linkedin = $_POST['linkedin'];
        if(!empty($linkedin)){
            $db->run("UPDATE `content` SET `link` = '$linkedin' WHERE `title` = 'linkedin'");
        }
        else
        {
            $db->run("UPDATE `content` SET `link` = 'لینک مورد نظر را وارد نمایید' WHERE `title` = 'linkedin'");
        }
        ?>
        <script>
